...
printf "%s\t%s\t%s\t%sf\n" "fileName" "seq_Len" "totalReads" "seqDepth" >> /scratch/kh31516/Gtex/Blood/WGS_normal_blood_result/Gtex_blood_coverage_summary.txt
while read line;
do 
    ...
    printf  "%s\t%d\t%d\t%4f\n" "${line}" "$length" "$total" "$coverage" >> /scratch/kh31516/Gtex/Blood/WGS_normal_blood_result/Gtex_blood_coverage_summary.txt
