...
printf "%s\t%s\t%s\t%sf\n" "fileName" "seq_Len" "totalReads" "seqDepth" >> /scratch/kh31516/TCGA/Stomach_original/Stomach/seqDepthSummary.txt
while read line;
do 
    ...
    printf  "%s\t%d\t%d\t%2f\n" "${line}" "$length" "$total" "$coverage" >> /scratch/kh31516/TCGA/Stomach_original/Stomach/seqDepthSummary.txt
