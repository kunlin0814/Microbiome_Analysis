...
with open ("/Volumes/Research_Data/Microbiome_analysis/CRC_combine_with_cufOff/TCGA_CRC_Gtex_significant_species_summary.txt",'r')as f:
    file = f.read()
    median = file.split('\n')[:-1]

## input file 2, which is the file contains the correlation information ##
with open("/Volumes/Research_Data/Microbiome_analysis/CRC_combine_with_cufOff/CORR/CORR_blood_species.result", 'r')as f:
    file1 = f.read()
    corr = file1.split('\n')[:-1]
    
    
overlap = open ("/Volumes/Research_Data/Microbiome_analysis/CRC_combine_with_cufOff/median_corr_overlap_withcutoff.txt",'w')    
  
overlap.write('TCGA_Gtexoverlap\tTCGA_overlap_pvalue\tTCGA_sample_ratio\tTCGA_species_median\tGtex_sample_ratio\tGtex_species_median\tMedian_diff\tSpecies\tMIC\tspearman\tpVspearman\tpearson\tpVpearson\tCase\n')
