...
threshold_count = [9.12] # the threshold_count here is the total species enrichment, here is the place we need to change
Categories = ['Phylum','Family','Species']
for category in Categories :
    with open ('/scratch/kh31516/Gtex/Blood/WGS_normal_blood_result/source/'+category+'_gt_cutoff_blood_gtex_fill0.txt' ,'r') as f:
        total_file = f.read()
    ...
    for file_name in total_name:
        total_read_file=file_name.split('/')[8] ## here we need to find the file name location, the number[8] will change depend how you put in your folder
        with open ('/scratch/kh31516/Gtex/Blood/WGS_normal_blood_result/results/gt_cutoff/'+total_read_file+'/'+total_read_file+'-TotalReads','r') as f1:
    ...
    total_share_species =[]
    with open ('/scratch/kh31516/TCGA/CRC/source/no_cut_micr_Gtex_'+category+'_overlap.txt','r') as f:
    ...
    for j in threshold_count :
        pass_files = []
        file_output = open('/scratch/kh31516/TCGA/CRC/source/Gtex_'+category+'files_didt_pass_threshold_count_'+str(j)+'_summary.txt','w')
        for i in each_file.keys():
            if each_file[i] > float(j):
                ...
                file_output.write(str(threshold)+'\t'+ 'theGtex_'+category+'_enrich is:\t' + str(each_file[i]) +'\n')
        summary={}
        file_species_summary = open('/scratch/kh31516/TCGA/CRC/source/Gtex_blood_all'+category+'_summary.txt','w')
        Species_With_cutoff =  open('/scratch/kh31516/TCGA/CRC/source/Gtex_blood_'+category+'_with_cutoff.txt','w')
        ...
        for pass_file in pass_files:
            ...
            with open ('/scratch/kh31516/Gtex/Blood/WGS_normal_blood_result/results/gt_cutoff/'+pass_file_name+'/'+pass_file_name+'-TotalReads','r') as f1:
