...
# Phylum_gt_cut_off_total_blood_CRC.txt
# Family_gt_cut_off_total_blood_CRC.txt
# Family_gt_cutoff_blood_gtex_fill0.txt
Categories = ['Phylum','Family','Species']
for category in Categories :
    with open ('/scratch/kh31516/TCGA/CRC/source/'+category+'_gt_cut_off_total_blood_CRC.txt' ,'r') as f: 
        TCGA_total_file=f.read()
        TCGA_total_name=TCGA_total_file.split('\n')[:-1]

    with open ('/scratch/kh31516/Gtex/Blood/WGS_normal_blood_result/source/'+category+'_gt_cutoff_blood_gtex_fill0.txt','r') as f1:
        Gtex_total_file = f1.read()
        Gtex_total_name = Gtex_total_file.split('\n')[:-1]
        
    TCGA_output = open('/scratch/kh31516/TCGA/CRC/source/TCGA_'+category+'_overlapSummary.txt' ,'w')
    ...
    GTex_output = open('/scratch/kh31516/TCGA/CRC/source/Gtex_'+category+'_overlapSummary.txt' ,'w')
    ...
    for i in TCGA_summary.keys(): 
        TCGA_output.write(category + '\t'+ str(i) + '\t'+  "ratio"+ '\t'+ str(TCGA_summary[i]/len(TCGA_total_name)))
        TCGA_output.write('\n')
    ...
    for i in Gtex_summary.keys(): 
        GTex_output.write(category + '\t'+ str(i) + '\t'+  "ratio"+ '\t'+ str(Gtex_summary[i]/len(Gtex_total_name)))
        GTex_output.write('\n')
    ...
    with open ('/scratch/kh31516/TCGA/CRC/source/Gtex_'+category+'_overlapSummary.txt' , 'r' )as f:
        Gtex_file = f.read()
    ...
    with open ('/scratch/kh31516/TCGA/CRC/source/TCGA_'+category+'_overlapSummary.txt', 'r' )as f1:
        TCGA_file = f1.read()
    ...
    g_overlap = open ('/scratch/kh31516/TCGA/CRC/source/no_cut_micr_Gtex_'+category+'_overlap.txt', 'w')
    g_overlap.write('Gtex_overlap_'+category+'\t'+'Gtex_blood_ratio')
    g_overlap.write('\n')
    ...
    T_overlap = open ('/scratch/kh31516/TCGA/CRC/source/no_cut_micr_TCGA_'+category+'_overlap.txt', 'w')
    T_overlap.write('TCGA_overlap_'+category+'\t'+ 'TCGA_blood_ratio')
    T_overlap.write('\n')
    ...
    G_uniq = open ('/scratch/kh31516/TCGA/CRC/source/no_cut_micr_Gtex_'+category+'_uniq.txt', 'w')
    G_uniq.write('Gtex_uniq_'+category+'\t'+ 'Gtex_blood_ratio')
    G_uniq.write('\n')
    ...
    T_uniq.write('TCGA_uniq_'+category+'\t'+'TCGA_blood_ratio')
    T_uniq.write('\n')
