...
for category in Categories :
    ...
    for file_name in total_name:
        ...
        with open ('/scratch/kh31516/TCGA/CRC/gt_cutoff/'+total_read_file+'/'+total_read_file+'-TotalReads','r') as f1:
    ...
    for j in threshold_count :
        pass_files = []
        file_output = open('/scratch/kh31516/TCGA/CRC/source/TCGA_'+category+'_files_didt_pass_threshold_count_'+str(j)+'_summary.txt','w')
        for i in each_file.keys():
            if each_file[i] > float(j):
                ...
                file_output.write(str(threshold)+'\t'+ 'theTCGA_CRC_'+category+'_enrich is:\t' + str(each_file[i]) +'\n')
        summary={}
        file_species_summary = open('/scratch/kh31516/TCGA/CRC/source/TCGA_blood_all'+category+'_summary.txt','w')
        Species_With_cutoff =  open('/scratch/kh31516/TCGA/CRC/source/TCGA_blood_'+category+'_with_cutoff.txt','w')
        ...
        for pass_file in pass_files:
            ...
            with open ('/scratch/kh31516/TCGA/CRC/gt_cutoff/'+pass_file_name+'/'+pass_file_name+'-TotalReads','r') as f1:
