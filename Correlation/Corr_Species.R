...
conn <- file("/scratch/kh31516/TCGA/colon/results/CORR/Species/blood",open="r") ## file names of all samples 
linn <-readLines(conn)
result<-cbind("Species", "MIC", "spearman","pVspearman","pearson","pVpearson","Case") # header

## output file name and location
write.table(result,append = TRUE,col.names=F,row.names=F,sep="\t",quote=F,file="/scratch/kh31516/TCGA/colon/results/CORR/CORR_blood_species.result")
...
conn <- file("/scratch/kh31516/TCGA/colon/results/CORR/Species/adjacent",open="r")
linn <-readLines(conn)
result<-cbind("Species", "MIC", "spearman","pVspearman","pearson","pVpearson","Case")
write.table(result,append = TRUE,col.names=F,row.names=F,sep="\t",quote=F,file="/scratch/kh31516/TCGA/colon/results/CORR/CORR_adjacent_species.result")
